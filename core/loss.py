...
class BoneSymmMSELoss(nn.Module):
    def __init__(self, num_joints=17):
        ...
        valid_pairs = [['Hip', 'Knee'], ['Knee', 'Foot'],
                       ['Shoulder', 'Elbow'], ['Elbow', 'Wrist']]
        self.joint_map = {k: v for (v, k) in enumerate(H36_joints)}
        self.num_joints = 17
        self.valid_pairs = [(side + joint[0], side + joint[1])
                            for joint in valid_pairs for side in ("L", "R")]
        self.valid_pairs += [('Neck', 'LShoulder'), ('Neck', 'RShoulder'),
                             ('Hip', 'LHip'), ('Hip', 'RHip')]
    ...
    def forward(self, coordinates):
        lengths = torch.stack(
            [self._get_length(pair, coordinates) for pair in self.valid_pairs])
