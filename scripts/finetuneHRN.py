...
import models
import models.hrnet
import config.finetune as config
import config.hrnet
...
def get_new_HR2D():
	...
	model.init_weights(config.hrnet.PRETRAINED, config.USE_GPU)
...
def main():
	...
	train_set = DataSet(config.DATA_PATH, image_transforms=normalize, num_joints=17, heatmap2D=True)
	...
	torch.save(model.state_dict(), os.path.join(config.LOG_PATH, config.NAME))
	logger.info("[+] Saved final model at %s" % os.path.join(config.LOG_PATH, config.NAME))
