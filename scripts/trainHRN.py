...
logging.basicConfig(filename=config.finetune.LOG_NAME,
                    filemode='a',
                    format=logFormatter,
                    level=logging.DEBUG)
...
def train(model: nn.Module, train_loader: DataLoader) -> None:
    model.train()
    optimizer = getattr(optim, config.finetune.OPTIMIZER)(
        model.parameters(),
    ...
    for epoch in range(config.finetune.NUM_EPOCHS):
        for batch_idx, sample in enumerate(train_loader):
            ...
            if overall_iter % config.finetune.SAVE_ITER_FREQ == 0:
                torch.save(
                    model.state_dict(),
                    os.path.join(config.finetune.LOG_PATH,
                                 config.finetune.NAME))
...
def main():
    ...
    torch.save(model.state_dict(),
               os.path.join(config.finetune.LOG_PATH, config.finetune.NAME))
